...
# VPC Module (использует существующую подсеть с разными CIDR для dev/prod)
...
resource "yandex_vpc_subnet" "private" {
  ...
  zone           = var.zone
  v4_cidr_blocks = var.environment == "prod" ? ["10.7.0.0/24"] : ["10.6.0.0/24"]
}
EOF
...
module "vpc" {
  source            = "../modules/vpc"
  username          = "ayakurnov1983"  # Замените на ваши данные (фамилия, инициалы, год рождения)
  environment       = "dev"
...
module "security_groups" {
  ...
  username       = "ayakurnov1983"  # Замените на ваши данные
  environment    = "dev"
...
echo ""
echo "ВАЖНО: Замените 'ayakurnov1983' на ваши данные (фамилия, инициалы, год рождения) в файлах:"
echo "/opt/dev/main.tf"
echo "/opt/prod/main.tf"
echo ""
...
echo ""
echo "Перед запуском prod убедитесь, что dev успешно развернулся!"
